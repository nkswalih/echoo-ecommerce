...
const AccessoriesPage = () => {
  ...
  const all = products.filter((p) => isCategoryMatch(p.category, 'accessory'));
  const apple = all.filter((p) => isBrandMatch(p.brand, 'Apple'));
  const cases = all.filter(p => p.name?.toLowerCase().includes('case'));
  ...
  const other = all.filter(p =>
    !p.name?.toLowerCase().includes('case') &&
    !p.name?.toLowerCase().includes('earbud') &&
    !p.name?.toLowerCase().includes('airpod') &&
    !p.name?.toLowerCase().includes('headphone') &&
    !p.name?.toLowerCase().includes('audio') &&
    !isBrandMatch(p.brand, 'Apple')
  );
  ...
  if (loading) return (
    <div className="min-h-screen bg-gradient-to-br from-[#d9e8f5] via-[#e2ebf4] to-[#f4f7fa] pt-20 pb-16">
      <div className="max-w-[1500px] mx-auto px-4 sm:px-6 pt-6 pb-10">
        <TextLine className="w-32 h-3 mb-2" />
      ...
      <div className="max-w-[1500px] mx-auto px-4 sm:px-6">
        ...
        <div className="mb-16">
          <div className="flex items-end justify-between mb-6">
            <div>
              <TextLine className="w-28 h-3 mb-1" />
              <TextLine className="w-44 h-10 mb-1" />
              <TextLine className="w-20 h-4" />
            </div>
          </div>
          <CategoryGridSkeleton count={4} />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#d9e8f5] via-[#e2ebf4] to-[#f4f7fa] pt-20 pb-16"
      ...
      <div className="max-w-[1500px] mx-auto px-4 sm:px-6 pt-6 pb-10">
        <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Echoo Accessories</p>
        <h1 className="text-4xl sm:text-6xl font-semibold tracking-tight text-gray-900 leading-none mb-3">Premium Accessories.</h1>
        <p className="text-gray-500 text-lg font-light">Essential Companions. <span className="text-gray-900 font-medium">Enhance your device experience.</span></p>
      </div>
      <div className="max-w-[1500px] mx-auto px-4 sm:px-6 mb-10"><div className="h-px bg-white/60 rounded-full"></div></div>
      <div className="max-w-[1500px] mx-auto px-4 sm:px-6">
        <Section title="Apple Accessories" subtitle="Apple" products={apple} idx={appleIdx} setIdx={setAppleIdx} />
        <Section title="Phone Cases" subtitle="Protection" products={cases} idx={casesIdx} setIdx={setCasesIdx} />
        <Section title="Audio & Headphones" subtitle="Sound" products={audio} idx={audioIdx} setIdx={setAudioIdx} />
        <Section title="Other Accessories" subtitle="Everything Else" products={other} idx={otherIdx} setIdx={setOtherIdx} />
        {!all.length && (
          <div className="text-center py-24"><p className="text-2xl font-medium text-gray-500">No accessories available.</p></div>
        )}
      </div>
